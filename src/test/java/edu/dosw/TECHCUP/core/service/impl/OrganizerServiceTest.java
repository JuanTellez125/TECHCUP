...
class OrganizerServiceTest {
    ...
    @InjectMocks OrganizerService service;
    ...
    void createUser_success() {
        ...
        verify(userRepository).save(argThat(e -> e.getUserType() == Role.ORGANIZER));
    }
    ...
    @Test
    void getOrganizerById_found_returnsUser() {
        UserEntity entity = buildUserEntity(1L);
        User model = buildUserModel();
        ...
        User result = service.getOrganizerById(1L);

        assertThat(result).isEqualTo(model);
    }

    @Test
    void getOrganizerById_notFound_throws() {
        when(userRepository.findById(99L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> service.getOrganizerById(99L))
                .isInstanceOf(UserNotFoundException.class);
    }
